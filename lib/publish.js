...
module.exports = (ssb, meta, tarball, cb) => {
  ...
  function isNew(feedId, moduleName, version, cb) {
    current(feedId, meta.name, (err, curr) => {
      if (err) return cb(err)
      debug(`current version: ${curr ? curr.version : 'none'}, new version: ${version}`)
      if (curr && !semver.gt(version, curr.version)) return cb(new Error(`The version you try to publish (${version}) must be greater than the current version (${curr.version})`))
      return cb(null, curr)
    })
  }
  ...
  ssb.whoami( (err, feed)=>{
    ...
    isNew(feed.id, meta.name, version, (err, curr)=>{
      ...
      upload(curr ? curr.key : null, tarball, meta, cb)
    })
  })
